...
#####################################################################
# Implement merge and transfer function for reaching definition pass
#####################################################################
def reachdef_transfer(block, in_vals):
    out_vals = dict(in_vals)
    ...
    for instr in block:
        ...
        if "dest" in instr:
            ...
            if dup_assign:
                del out_vals[key_to_be_del]
            out_vals[str(instr)] = instr["dest"]
    return out_vals

def reachdef_merge(vals_list):
    out_vals = {}
...
ANALYSES = {
    ...
    # A simple reach definition pass.
    # returns a dictionary with reached variables - key: block name, value: variable name
    'reachdef': Analysis(
        True,
        init={},
        merge=reachdef_merge,
        transfer=reachdef_transfer,
    ), 
}
